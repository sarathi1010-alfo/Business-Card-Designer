const fs = require('fs');
const filePath = 'src/app/use-cases/[use-case]/page.tsx';
...
// Update UseCasePage
const useCasePageSearch = `export default async function UseCasePage({ params }: PageProps) {
  const resolvedParams = await params;
  const rawUseCase = resolvedParams['use-case'].replace(/-digital-business-card/g, '').replace(/-digital-card/g, '');
...
const useCasePageReplace = `export default async function UseCasePage({ params }: PageProps) {
  const resolvedParams = await params;
  const rawUseCase = resolvedParams['use-case'].replace(/-digital-business-card/g, '').replace(/-digital-card/g, '');
...
content = content.replace(useCasePageSearch, useCasePageReplace);
...
const renderContentSearch = `        <h2 className="text-3xl font-semibold mt-12 mb-6">Master Your {useCaseTitle} Networking</h2>
        <p>
...
const renderFaqSearch = `        <h2 className="text-3xl font-semibold mt-12 mb-6">Frequently Asked Questions</h2>
...
const renderFaqReplace = `        <h2 className="text-3xl font-semibold mt-12 mb-6">{faqHeader}</h2>
        ...
        <p>{a2}</p>`;
content = content.replace(renderFaqSearch, renderFaqReplace);
